...
export function startClient(url) {
  ...
  try {
    ...
    initRl.question(chalk.cyan("Enter your username (press Enter for a random anime character): "), (usernameInput) => {
      initRl.close();

      let wsUrl = rawWsUrl;
      if (usernameInput.trim()) {
        wsUrl += (wsUrl.includes('?') ? '&' : '?') + `username=${encodeURIComponent(usernameInput.trim())}`;
      }
          ...
          rl.on("line", (input) => {
            try {
              ...
              if (ws.readyState === WebSocket.OPEN) {
                ...
                console.log(' '.repeat(paddingLength) + authorLabel + chalk.gray(' : ') + textMessage);
                console.log(); // Add an extra newline after user's message
              } else {
